...
//PARAMETRE INFORMANT L'ANNEE COURRANTE D'ETUDE :
    ...
    for ($i = 0; $i < count($lines['VEVENT']); $i++){
        ...
        if ((int)substr($StringAnnee, 4, 2)<=9){//ATTENTION, a modifier en fin de projet
        ...
        for($l=0; $l < strlen($StringClasseProf); $l++){
            ...
            if($parcoureur=='S' and $StringClasseProf[$l-1]=='-'){
                $parcoureur='';
                continue;
            }
            ...
            if($StringClasseProf[$l]=='-'){
                ...
                if ($parcoureur=='EPU'){
                    //echo('Je passe par EPU');
                    //echo('<br/>');
                    for ($j = 0; $j < count($EPU); $j++){
        ...
        for ($l=0; $l<count($filiere); $l++){
            $promosql = $anneecourante+5-$annee[$l];
            $dataetudiantconcernes = $bdd->query('SELECT idetudiant FROM '.$bdName.'.etudiant WHERE (promo='.$promosql.' AND filiere="'.$filiere[$l].'")');
